...
should return "1A1B".
You may assume that the secret number and your friend's guess only contain digits, and their lengths
are always equal.
*/
...
public:
    string getHint(string secret, string guess) {
        std::vector<int> secret_nums_count(128, 0);
        std::vector<int> guess_nums_count(128, 0);

        for(int i = 0; i < secret.size(); i++) {
            secret_nums_count[int(secret[i])]++;
            guess_nums_count[int(guess[i])]++;
        }

        int bulls = 0;
        int cows = 0;
        for(int i = 0; i < guess.size(); i++) {
            //这一步其实可以合并到前面初始化哈希表的循环里，以省掉这次遍历
            if(secret[i] == guess[i]) {
                secret_nums_count[int(guess[i])]--;
                guess_nums_count[int(guess[i])]--;
                bulls++;
            }
        }
        for(int i = 0; i < guess.size(); i++) {
            if((secret_nums_count[int(guess[i])] > 0)
                &&(guess_nums_count[int(guess[i])] > 0)) {
                secret_nums_count[int(guess[i])]--;
                guess_nums_count[int(guess[i])]--;
                cows++;
            }
